...
namespace EternityMod.Core.Globals.Items;

public class EternityGlobalItem : GlobalItem
{
    ...
    public delegate bool PreDrawTooltipLineDelegate(Item item, DrawableTooltipLine line, ref int yOffset);

    public static event PreDrawTooltipLineDelegate PreDrawTooltipLineEvent;

    public static event Action<Player> UpdateArmorSetEvent;
    ...
    {
        if (PreDrawTooltipLineEvent is null)
            return true;

        bool result = true;
        foreach (Delegate d in PreDrawTooltipLineEvent.GetInvocationList())
            result &= ((PreDrawTooltipLineDelegate)d).Invoke(item, line, ref yOffset);
    ...
    {
        UpdateArmorSetEvent?.Invoke(player);
    }
}
